...
void x10rf::RFXmeter(uint8_t rfxm_address, uint8_t rfxm_packet_type,
  ...
  x10buff[1] =
      (~x10buff[0] & 0xF0) +
      (x10buff[0] &
       0xF); // Calculate byte1 (byte 1 complement upper nibble of byte0)
  ...
  uint8_t parity = ~(((x10buff[0] & 0XF0) >> 4) + (x10buff[0] & 0XF) +
                     ((x10buff[1] & 0XF0) >> 4) + (x10buff[1] & 0XF) +
                     ((x10buff[2] & 0XF0) >> 4) + (x10buff[2] & 0XF) +
                     ((x10buff[3] & 0XF0) >> 4) + (x10buff[3] & 0XF) +
                     ((x10buff[4] & 0XF0) >> 4) + (x10buff[4] & 0XF) +
                     ((x10buff[5] & 0XF0) >> 4));
  x10buff[5] = (x10buff[5] & 0xf0) + (parity & 0XF);
  SendCommand(x10buff, BUFFER_SIZE);
}
...
void x10rf::SEND_HIGH() { digitalWrite(_tx_pin, HIGH); }

void x10rf::SEND_LOW() { digitalWrite(_tx_pin, LOW); }
